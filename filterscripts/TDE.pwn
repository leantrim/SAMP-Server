...
new
    ...
    line[800], bool:MouseCursor, bool:PixelLock
;
	...
	if(!strcmp(cmdtext, "/tde", true, 4))
	{
	    if(ProjectEditor == playerid)
	    {
	        if(strlen(cmdtext) >= 6)
			{
				...
				{
				    SendClientMessage(playerid, -1, "{e2b960}TDEditor: {FFFFFF}/tde box - /tde exit - /tde cursor.");
				    return 1;
				}
				...
				if( (!strcmp(cmdtext[5], "box", true)) || (!strcmp(cmdtext[5], "icons", true)) || (!strcmp(cmdtext[5], "adjust", true)) )
				{
				    if(EditMode == EDITMODE_ADJUST)
				    {
				        if(!VirtualKeys[26][KEY_PRESSED]) //Left Mouse Button
						{
				        	...
				        	for(new i = 1; i < sizeof(TDE_Menu); i++)
				        	{
				 		    ...
				 		    TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_NONE");
				 		    EditMode = EDITMODE_NONE;
				 		    IsPSel = true;
					        return 1;
						}
				    ...
				    IsPSel = false;
					TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_ADJUST");
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	if(clickedid == TDE_Menu[9])
	{
		...
		{
			TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_POSITION");
			EditMode = EDITMODE_POSITION;
		}
		else TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_NONE"), EditMode = EDITMODE_NONE;
		return 1;
	}
	if(clickedid == TDE_Menu[10])
	{
		...
		{
			TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_SIZE");
	...
	if(clickedid == TDE_Menu[11] || clickedid == TDE_Menu[35])
	{
	    ...
	    */
		...
		{
			TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_TEXT");
	...
	if(clickedid == TDE_Menu[14])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_COLOR");
	...
	if(clickedid == TDE_Menu[15])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_BGCOLOR");
	...
	if(clickedid == TDE_Menu[32])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_BOXCOLOR");
	...
	if(clickedid == TDE_Menu[16])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_LETTERSIZE");
	...
	if(clickedid == TDE_Menu[17])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_OUTLINE");
	...
	if(clickedid == TDE_Menu[18])
	{
		...
		{
	    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_SHADOW");
	...
	if(clickedid == TDE_Menu[12] || clickedid == TDE_Menu[13] || clickedid == TDE_Menu[24] || clickedid == TDE_Menu[25] || clickedid == TDE_Menu[26] || clickedid == TDE_Menu[27])
	{
	    if(TDE_TextDrawGetFont(ProjectTD[EditIndex][ETextDrawID]) == 5)
	    {
	        if(EditMode != EDITMODE_MODELS)
			{
		    	...
		    	TDE_TextDrawSetString(TD_Status, "EditMode: EDITMODE_MODELS");
...
TDE_CALLBACK:OnCursorPositionChange(playerid, NewX, NewY)
{
    ...
    switch(EditMode)
    {
		...
		case EDITMODE_SIZE:
        {
			...
			{
			    ProjectTD[EditIndex][ETextDrawTextX] -= floatdiv(floatsub(CursorOX, NewX), ScreenWidth) * 640.0;
				if(TDE_TextDrawGetFont(ProjectTD[EditIndex][ETextDrawID]) == 4 || TDE_TextDrawGetFont(ProjectTD[EditIndex][ETextDrawID]) == 5) ProjectTD[EditIndex][ETextDrawTextY] -= floatdiv(floatsub(CursorOY, NewY), ScreenHeight) * 448.0;
...
TDE_CALLBACK:ReSizeTextDrawEDITOR()
{
    ...
    if(VirtualKeys[29][KEY_PRESSED]) //LEFT
	{
	    ...
	    new SpeedSTR[64];
 		format(SpeedSTR, 64, "%.4f, %.4f", ProjectTD[EditIndex][ETextDrawTextX], ProjectTD[EditIndex][ETextDrawTextY]);
 		if( (TDE_TextDrawIsBox(ProjectTD[EditIndex][ETextDrawID]) && ProjectTD[EditIndex][ETextDrawLetterX] == 0.0)  ) format(SpeedSTR, 64, "%.4f, %.4f", ProjectTD[EditIndex][ETextDrawTextX], ProjectTD[EditIndex][ETextDrawLetterY]);
		ShowInfoDraw(SpeedSTR, 1000);
	    EditSizeTDTimer = SetTimer("ReSizeTextDrawEDITOR", 25, 0);
	    return 1;
	}
...
stock CreateMenuTextDraws()
{
	...
	TDE_TextDrawCreate(TD_Status, 2.0, 404.86, "EditMode: EDITMODE_NONE");
...
stock DuplicateTextDraw(Index, ToIndex)
{
	...
	new Float:p[2];
	TDE_TextDrawGetFontSize(ProjectTD[Index][ETextDrawID], p[0], p[1]);
	...
	TDE_TextDrawLetterSize(ProjectTD[ToIndex][ETextDrawID], p[0], p[1]);
	TDE_TextDrawAlignment(ProjectTD[ToIndex][ETextDrawID], TDE_TextDrawGetAlignment(ProjectTD[Index][ETextDrawID]));
	TDE_TextDrawColor(ProjectTD[ToIndex][ETextDrawID], ProjectTD[ToIndex][ETextDrawColor]);
	TDE_TextDrawUseBox(ProjectTD[ToIndex][ETextDrawID], TDE_TextDrawIsBox(ProjectTD[Index][ETextDrawID]));
	TDE_TextDrawTextSize(ProjectTD[ToIndex][ETextDrawID], ProjectTD[ToIndex][ETextDrawTextX], ProjectTD[ToIndex][ETextDrawTextY]);
	...
	TDE_TextDrawFont(ProjectTD[ToIndex][ETextDrawID], TDE_TextDrawGetFont(ProjectTD[Index][ETextDrawID]));
	TDE_TextDrawSetProportional(ProjectTD[ToIndex][ETextDrawID], TDE_TextDrawIsProportional(ProjectTD[Index][ETextDrawID]));

	if(TDE_TextDrawGetFont(ProjectTD[Index][ETextDrawID]) == 5)
 	{
		...
		if(IsVehicle(ProjectTD[ToIndex][ETextDrawModelid])) TDE_TextDrawSetPreviewVehCol(ProjectTD[ToIndex][ETextDrawID], ProjectTD[EditIndex][ETextDrawVehCol1], ProjectTD[EditIndex][ETextDrawVehCol2]);
 	}
 	else
 	{
 	    if(EditMode == EDITMODE_MODELS)
...
stock ExportProject()
{
	...
	format(line, sizeof(line), "\r\nTDEditor V1.15 by adri1"); fwrite(ExportIO, line);
	fclose(ExportIO);
...
{
	TDE_TextDrawCreate(InfoDraw, 320.000000, 217.000000, "320.000,_320.000~n~");
	TDE_TextDrawLetterSize(InfoDraw, 0.659499, 2.686399);
	TDE_TextDrawAlignment(InfoDraw, 2);
	TDE_TextDrawColor(InfoDraw, -1);
	TDE_TextDrawSetShadow(InfoDraw, 0);
	TDE_TextDrawSetOutline(InfoDraw, 2);
	TDE_TextDrawBackgroundColor(InfoDraw, 255);
	TDE_TextDrawFont(InfoDraw, 2);
	TDE_TextDrawSetProportional(InfoDraw, 1);
	TDE_TextDrawSetShadow(InfoDraw, 0);
	return 1;
}
...
static ShowInfoDraw(text[], time)
{
	...
	TDE_TextDrawDestroy(InfoDraw);
	CreateInfoDraws();
	TDE_TextDrawSetString(InfoDraw, text);
    TDE_TextDrawShowForPlayer(ProjectEditor, InfoDraw);
